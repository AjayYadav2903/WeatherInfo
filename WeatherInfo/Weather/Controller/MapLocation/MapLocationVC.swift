...
class MapLocationVC: UIViewController {
    ...
    lazy var bookMarkBtn : UIButton = {
        ...
        button.addTarget(self, action: #selector(bookmarkLocation(_:)), for: .touchUpInside)
        return button
    }()
    ...
    func setUpView(){
        ...
        self.addressPopView.addSubview(bookMarkBtn)

        bookMarkBtn.snp.makeConstraints { (makeBtn) in
            makeBtn.trailing.equalTo(self.addressPopView.snp.trailing).offset(-10)
            makeBtn.top.equalTo(self.addressPopView.snp.top).offset(10)
        }
...
extension MapLocationVC {
        ...
        if let latitude = selectedPlace?.coordinate.latitude, let longitude = selectedPlace?.coordinate.longitude{
            ...
            DispatchQueue.main.async {
                if self.selectedPlaceMark != nil {
                    ...
                    self.latitude = "\(center?.latitude ?? 0)"
                    self.longitude = "\(center?.longitude ?? 0)"
                }
            }
        }else{
    ...
    //Back Button Action
    @objc func bookmarkLocation(_ sender : UIButton){
        let items = NSEntityDescription.insertNewObject(forEntityName: "LocationsCoreModel", into: self.context) as! LocationsCoreModel
        items.address = self.addressLabel.text
        items.latitude = self.latitude
        items.longitude = self.latitude
        do {
            try self.context.save()
            let alert = UIAlertController(title: "Alert", message: "Location Bookmarked", preferredStyle: .alert)
